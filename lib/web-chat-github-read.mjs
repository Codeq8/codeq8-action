...
import { createAsyncTtlCache } from "./async-ttl-cache.mjs";

const REPOSITORY_PATTERN = /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/;
const BRANCH_NAME_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._/-]{0,254}$/;
const GITHUB_REPOSITORIES_PER_PAGE = 100;
const GITHUB_PULL_REQUEST_COMMIT_METADATA_CACHE_TTL_MS = 60 * 1000;
const GITHUB_PULL_REQUEST_METADATA_CACHE_TTL_MS = 60 * 1000;
const GITHUB_ISSUE_METADATA_CACHE_TTL_MS = 60 * 1000;
...
const githubPullRequestCommitMetadataCache = createAsyncTtlCache({
  ttlMs: GITHUB_PULL_REQUEST_COMMIT_METADATA_CACHE_TTL_MS,
});
const githubPullRequestMetadataCache = createAsyncTtlCache({
  ttlMs: GITHUB_PULL_REQUEST_METADATA_CACHE_TTL_MS,
});
const githubIssueMetadataCache = createAsyncTtlCache({
  ttlMs: GITHUB_ISSUE_METADATA_CACHE_TTL_MS,
});
...
function buildGitHubPullRequestCommitMetadataCacheKey({
  githubLogin = "",
  repository = "",
  pullRequestNumber = 0,
  limit = 250,
  ...
  return [
    normalizeText(githubLogin).toLowerCase(),
    normalizedRepository.toLowerCase(),
    String(normalizedPullRequestNumber),
    String(normalizedLimit),
  ].join("::");
}

function buildGitHubPullRequestMetadataCacheKey({
  githubLogin = "",
  repository = "",
  pullRequestNumber = 0,
}) {
...
function buildGitHubIssueMetadataCacheKey({
  githubLogin = "",
  repository = "",
  issueNumber = 0,
}) {
  ...
  return [
    normalizeText(githubLogin).toLowerCase(),
    normalizedRepository.toLowerCase(),
    String(normalizedIssueNumber),
  ].join("::");
}

export function clearGitHubMetadataReadCaches() {
  githubPullRequestCommitMetadataCache.clear();
  githubPullRequestMetadataCache.clear();
  githubIssueMetadataCache.clear();
}
...
export function githubRepositoryAccessErrorResponse({
  ...
  if (response.status === 404) {
    return Response.json(
      {
        ok: false,
        error: `Codeq8 couldn't find ${normalizedRepository} on GitHub.`,
...
export async function fetchGitHubPullRequestMetadata({
  ...
  return await githubPullRequestMetadataCache.getOrLoad(
    buildGitHubPullRequestMetadataCacheKey({
      githubLogin,
      repository: normalizedRepository,
    ...
    {
      shouldCache: (value) => Boolean(value?.ok),
    },
  );
}
...
export async function fetchGitHubIssueMetadata({
  ...
  return await githubIssueMetadataCache.getOrLoad(
    buildGitHubIssueMetadataCacheKey({
      githubLogin,
      repository: normalizedRepository,
...
export async function fetchGitHubPullRequestCommitMetadata({
  ...
  const cacheKey = buildGitHubPullRequestCommitMetadataCacheKey({
    githubLogin,
    repository: normalizedRepository,
    pullRequestNumber: normalizedPullRequestNumber,
  ...
  return githubPullRequestCommitMetadataCache.getOrLoad(
    cacheKey,
    async () => {
    ...
    {
      shouldCache: (result) => result?.ok === true,
    },
  );
}
