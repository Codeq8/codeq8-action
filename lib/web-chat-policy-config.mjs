...
const CODEQ8_CONFIG_CACHE_TTL_MS = 60 * 1000;
const codeq8ConfigAtRefCache = createAsyncTtlCache({
  ttlMs: CODEQ8_CONFIG_CACHE_TTL_MS,
});
...
function buildCodeq8ConfigAtRefCacheKey({ repository, ref }) {
  const normalizedRepository = normalizeRepository(repository);
  const normalizedRef = normalizeBranchName(ref);
  if (!normalizedRepository || !normalizedRef) {
    return "";
  }
  return `${normalizedRepository.toLowerCase()}@${normalizedRef}`;
}

export function clearCodeq8ConfigReadCache() {
  codeq8ConfigAtRefCache.clear();
}
...
export async function fetchCodeq8ConfigAtRef({
  ...
  return await codeq8ConfigAtRefCache.getOrLoad(
    buildCodeq8ConfigAtRefCacheKey({
