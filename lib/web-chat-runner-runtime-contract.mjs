...
export const WEB_CHAT_RUNNER_AUXILIARY_REPOSITORIES_CAPABILITY =
...
export function supportsAuxiliaryRepositories(manifest) {
  const parsed = webChatRunnerRuntimeManifestResponseSchema.safeParse(manifest);
  if (!parsed.success) {
    return false;
  }
  ...
  return capabilitySet.has(WEB_CHAT_RUNNER_AUXILIARY_REPOSITORIES_CAPABILITY);
}
