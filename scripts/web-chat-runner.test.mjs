...
test("persistWorkspaceProgress creates missing branch-and-pr pull requests through the runner", async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "codeq8-action-auto-pr-"));
  ...
  globalThis.fetch = async (url, init) => {
    ...
    return Response.json({
      number: 43,
      title: body?.title,
      html_url: "https://github.com/Codeq8/codeq8-action/pull/43",
    });
  };
  ...
  try {
    ...
    git(workspacePath, ["checkout", "-b", "feature/auto-pr", "origin/main"]);
    ...
    git(workspacePath, ["push", "-u", "origin", "feature/auto-pr"]);
    ...
    const result = await persistWorkspaceProgress({
      ...
      baselineState: null,
      threadTitle: "Fix PR body formatting",
      threadId: "wct_123",
      runId: "wcr_123",
      assistantMessage: "Implemented runner-owned PR creation.\n\nValidation passed.",
    });

    assert.equal(result.error, "");
    assert.equal(result.pullRequestNumber, 43);
    assert.equal(result.pullRequestUrl, "https://github.com/Codeq8/codeq8-action/pull/43");
    assert.equal(calls.length, 2);
    assert.equal(calls[0]?.method, "GET");
    assert.equal(calls[1]?.method, "POST");
    assert.match(calls[1]?.url, /^https:\/\/api\.github\.com\/repos\/Codeq8\/codeq8-action\/pulls$/);
    assert.deepEqual(calls[1]?.body, {
      title: "Fix PR body formatting",
      head: "Codeq8:feature/auto-pr",
      base: "main",
      body: [
        "## Summary",
        "Implemented runner-owned PR creation.\n\nValidation passed.",
        "",
        "## Source",
        "- Thread: wct_123",
        "- Run: wcr_123",
