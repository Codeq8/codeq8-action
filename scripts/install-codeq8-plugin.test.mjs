...
test("Codeq8 plugin install removes obsolete Codeq8-managed skills only", async () => {
  await withTempInstallRoot(async ({ codexHome, env }) => {
    ...
    const result = await syncCodeq8PluginInstall({
      repoRoot: process.cwd(),
      env,
      sourceRef: "public-action-sha-cleanup",
      now: FIXED_NOW,
    });

    assert.equal(result.ok, true);
    assert.equal(await pathExists(obsoleteSkillPath), false);
    assert.equal(await pathExists(userSkillPath), true);
    assert.equal(result.targets.includes("removed-skill:codeq8-child-threads"), true);
  });
});
