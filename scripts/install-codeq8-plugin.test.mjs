...
import {
  ...
  CODEQ8_PLUGIN_MCP_SKILL_CAPABILITY,
  CODEQ8_PLUGIN_NAME,
...
test("Codeq8 plugin install syncs marked plugin, skill, and marketplace state", async () => {
  await withTempInstallRoot(async ({ homePath, codexHome, env }) => {
    ...
    for (const skillName of CODEQ8_PLUGIN_PUBLIC_SKILLS) {
      ...
      assert.equal(skillMarker.plugin_version, "0.5.6");
      assert.equal(await pathExists(path.join(skillPath, "SKILL.md")), true);
    }
    assert.equal(await pathExists(path.join(codexHome, "skills", "codeq8-coordinator")), false);
...
test("Codeq8 plugin bundles onboarding and MCP runtime skills", async () => {
  ...
  assert.equal(bundledSkillNames.includes("codeq8-coordinator"), false);
...
test("Codeq8 plugin run-behavior skills preserve the migration contract", async () => {
  ...
  const mcpSource = await readSkill("codeq8-mcp");
  ...
  assert.match(onboardingSource, /identify the repo skill that should guide future runs before\s+implementing durable repo work or relying on a reusable repo procedure/);
  assert.match(onboardingSource, /If it\s+exists, read and use it/);
  assert.match(onboardingSource, /create a focused owner skill only when future runs need a durable\s+owner boundary, checklist, validation pattern, or reusable debugging\s+procedure/);
  assert.match(onboardingSource, /Anchor the durable goal to the relevant skill name or path/);
  assert.match(onboardingSource, /skill update that meets\s+that durable threshold/);
  ...
  assert.match(onboardingSource, /current work reveals durable reusable guidance/);
  assert.match(onboardingSource, /stale or wrong guidance, user corrections that change\s+future behavior/);
  assert.match(onboardingSource, /question-only, clarification, or answer-only turn does not\s+justify skill edits by itself/);
  assert.match(onboardingSource, /if nothing durable was learned for future\s+runs, answer without skill changes or skill-only PRs/);
  assert.match(onboardingSource, /request to diagnose,\s+explain, or avoid product code does not by itself forbid skill upkeep, but\s+only when the durable-update threshold is met/);
  assert.match(onboardingSource, /forbid all file changes, PRs, or skill updates/);
  assert.match(onboardingSource, /Prefer\s+updating an\s+existing owner skill before creating one/);
  ...
  assert.match(onboardingSource, /Do not treat every user correction or follow-up question as skill-worthy/);
  assert.match(onboardingSource, /correction changes durable future procedure/);
  assert.match(onboardingSource, /Do not create separate managed threads\s+as hidden delegation/);
  assert.match(onboardingSource, /thread ids, decisive results, blockers, and cleanup status/);
  ...
  assert.match(
    mcpSource,
    /Do not treat `npx`, local browser auth caches, or vendor demo commands as the\s+default Codeq8 architecture/,
  );
  ...
  assert.match(readmeSource, /codeq8-mcp/);
  assert.match(readmeSource, /retired `codeq8-coordinator` skill is no longer bundled/);
  assert.match(pluginSource, /standalone coordinator skill is retired/);

  for (const source of [onboardingSource, mcpSource, pluginSource, readmeSource]) {
