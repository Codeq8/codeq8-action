...
function buildWorkspaceGitTokenHelperScript({
  ...
  return [
    ...
    "  const primaryWorkspaceToken = normalizeText(process.env.CODEX_GITHUB_WRITE_TOKEN || '');",
...
async function prepareRefreshingGitHubCliWrapper({
  ...
  const wrapperScript = [
    ...
    '  printf "%s/%s\\n" "$owner" "$repo"',
    "}",
    "normalize_gh_api_repository() {",
    '  value="${1:-}"',
    '  value="${value#https://api.github.com/}"',
    '  value="${value#http://api.github.com/}"',
    '  value="${value#api.github.com/}"',
    '  value="${value#/}"',
    '  case "$value" in repos/*/*) ;; *) return 1 ;; esac',
    '  value="${value#repos/}"',
    '  owner="${value%%/*}"',
    '  remainder="${value#*/}"',
    '  repo="${remainder%%/*}"',
    '  normalize_gh_repository "$owner/$repo"',
    "}",
    ...
    "  return 1",
    "}",
    "resolve_gh_api_repository_argument() {",
    "  expect_endpoint=0",
    "  for arg in \"$@\"; do",
    ...
    "      continue",
    "    fi",
    '    normalize_gh_api_repository "$arg" && return 0',
    "  done",
    "  return 1",
    "}",
    ...
    '  target_repo="$(resolve_gh_api_repository_argument "$@" || true)"',
    "fi",
